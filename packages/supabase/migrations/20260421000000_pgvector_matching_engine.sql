  ...
  exact_match AS (
    SELECT CASE
      ...
      WHEN EXISTS (
        ...
        WHERE LEFT(cnae, 2) = ANY(p_tender_cnae_classificados)
      ) THEN 1.0
      ELSE 0.0
    END AS score
  ),
    ...
    SELECT
      ...
      CASE
        ...
        ELSE LEAST(
          ...
          ) / GREATEST(array_length(cd.palavras_chave, 1), 1)::NUMERIC * 3
        )
      END AS s_keyword,
      CASE
    ...
    SELECT
      ...
      CASE
        ...
        ELSE LEAST(
          ...
          ) / GREATEST(array_length(cd.c_palavras, 1), 1)::NUMERIC * 3
        )
      END AS s_keyword,
      CASE
