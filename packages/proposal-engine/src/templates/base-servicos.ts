...
export function buildServicosSections(data: ProposalData): object[] {
  ...
  // --- Block 4: Price Table with Valor Mensal ---
  ...
  if (valor_mensal !== undefined && valor_mensal > 0) {
    summaryRows.push(
      new TableRow({
        children: [
          makeMergedCell('VALOR MENSAL', 5, totalMergedWidth, AlignmentType.RIGHT, true),
          makeCell(formatCurrencyBRL(valor_mensal), COL_WIDTHS[5], AlignmentType.RIGHT, true),
        ],
      }),
    );
  }
  ...
  summaryRows.push(
    new TableRow({
      children: [
        makeMergedCell('VALOR GLOBAL (12 meses)', 5, totalMergedWidth, AlignmentType.RIGHT, true),
