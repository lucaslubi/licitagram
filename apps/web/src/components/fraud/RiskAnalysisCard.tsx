...
function RiskAnalysisCardInner({ tenderId }: { tenderId: string }) {
  const { alerts, loading, analyzed } = useFraudAlerts(tenderId)
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {sorted.length > 0 ? (
            ...
            <span className="ml-auto text-xs font-normal text-gray-400 bg-[#2d2f33] px-2 py-0.5 rounded-full">
              Pendente
            </span>
          ) : null}
        </CardTitle>
        ...
        {sorted.length === 0 ? (
          analyzed ? (
            ...
            <div className="flex items-center gap-2 text-gray-400">
              <span className="text-lg">⏳</span>
              <span className="text-sm">Analise de risco pendente — sera processada automaticamente</span>
            </div>
          )
        ) : (
          <div className="space-y-2">
