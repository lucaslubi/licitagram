...
export function ScoreDonut({ distribution }: { distribution: { range: string; count: number; color: string; percentage: number }[] }) {
  ...
  const gradient = total > 0 ? `conic-gradient(${gradientStops})` : 'conic-gradient(#2d2f33 0% 100%)'

  return (
    <div className="flex items-center gap-6">
      <div className="relative w-32 h-32 shrink-0">
        <div className="w-full h-full rounded-full" style={{ background: gradient }} />
        <div className="absolute inset-3 bg-[#111214] rounded-full flex items-center justify-center">
      ...
      <div className="space-y-2 flex-1">
        {distribution.map(d => (
          <div key={d.range} className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-400 font-mono w-14">{d.range}</span>
      ...
      {data.map(([uf, count], i) => (
        <div key={uf} className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="h-6 bg-[#111214] rounded-full overflow-hidden">
              <div
      ...
      {data.map(([mod, count]) => (
        <div key={mod}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm text-gray-400 truncate max-w-[200px]">{mod}</span>
            <span className="text-sm font-semibold text-white ml-2">{count}</span>
          </div>
          <div className="h-2 bg-[#111214] rounded-full overflow-hidden">
            <div
...
export function DocumentHealth({ valid, expiring, expired }: { valid: number; expiring: number; expired: number }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-1 mb-4 h-3 rounded-full overflow-hidden bg-[#111214]">
    ...
    <div className="flex items-center gap-6">
      <div className="relative w-28 h-28 shrink-0">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r="45" fill="none" stroke="#2d2f33" strokeWidth="8" />
      ...
      <div>
        <p className="text-sm font-semibold text-white mb-2">Taxa de Vitoria</p>
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-500" />
            <span className="text-sm text-gray-400">{won} ganhas</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-red-400" />
            <span className="text-sm text-gray-400">{lost} perdidas</span>
          </div>
        </div>
      </div>
    </div>
