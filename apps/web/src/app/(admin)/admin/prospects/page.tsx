    ...
    <div>
      ...
      <form className="mb-6 flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        {(params.search || params.uf || params.porte || params.min_participacoes) && (
          <a href="/admin/prospects" className="px-4 py-2 border rounded-md text-sm text-gray-600 hover:bg-gray-50">
            Limpar
          </a>
        )}
      </form>
          ...
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="text-center px-3 py-3 font-medium text-gray-500">
                <a href={sortUrl('total_participacoes')} className="hover:text-gray-900">Partic.{sortIcon('total_participacoes')}</a>
              </th>
              <th className="text-center px-3 py-3 font-medium text-gray-500">
                <a href={sortUrl('total_vitorias')} className="hover:text-gray-900">Vitorias{sortIcon('total_vitorias')}</a>
              </th>
              <th className="text-center px-3 py-3 font-medium text-gray-500">
                <a href={sortUrl('win_rate')} className="hover:text-gray-900">Win Rate{sortIcon('win_rate')}</a>
              </th>
              <th className="text-right px-3 py-3 font-medium text-gray-500 hidden md:table-cell">
                <a href={sortUrl('valor_total_ganho')} className="hover:text-gray-900">Valor Ganho{sortIcon('valor_total_ganho')}</a>
              </th>
              <th className="text-center px-3 py-3 font-medium text-gray-500 hidden md:table-cell">
                <a href={sortUrl('desconto_medio')} className="hover:text-gray-900">Desc. Medio{sortIcon('desconto_medio')}</a>
              </th>
              <th className="text-left px-3 py-3 font-medium text-gray-500 hidden xl:table-cell">UFs Atuacao</th>
              <th className="text-left px-3 py-3 font-medium text-gray-500 hidden xl:table-cell">Modalidades</th>
              <th className="text-left px-3 py-3 font-medium text-gray-500 hidden 2xl:table-cell">
                <a href={sortUrl('ultima_participacao')} className="hover:text-gray-900">Ultima Partic.{sortIcon('ultima_participacao')}</a>
              </th>
            </tr>
          </thead>
            ...
            {(competitors || []).map((c) => {
              ...
              return (
                <tr key={c.cnpj} className="hover:bg-gray-50">
                  <td className="px-3 py-2.5 font-medium text-gray-900 max-w-[200px] truncate" title={c.razao_social || ''}>
                    {c.razao_social || '—'}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-gray-600 hidden lg:table-cell">{c.uf || '—'}</td>
                  <td className="px-3 py-2.5 text-gray-600 hidden xl:table-cell">{c.cnae_divisao || '—'}</td>
                  <td className="px-3 py-2.5 text-center font-semibold">{c.total_participacoes || 0}</td>
                  <td className="px-3 py-2.5 text-center text-emerald-700 font-semibold">{c.total_vitorias || 0}</td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`font-semibold ${
                      Number(c.win_rate) >= 0.5 ? 'text-emerald-700' : Number(c.win_rate) >= 0.2 ? 'text-amber-600' : 'text-gray-500'
                    }`}>
                  ...
                  <td className="px-3 py-2.5 text-center text-gray-600 hidden md:table-cell">
                    {formatPercent(c.desconto_medio)}
                  </td>
                  <td className="px-3 py-2.5 hidden xl:table-cell">
                    <div className="flex flex-wrap gap-0.5">
                      {ufsKeys.map((uf) => (
                        <span key={uf} className="text-[10px] px-1 py-0.5 bg-blue-50 text-blue-700 rounded">{uf}</span>
                      ))}
                  ...
                  <td className="px-3 py-2.5 hidden xl:table-cell">
                    <div className="flex flex-wrap gap-0.5 max-w-[150px]">
                      {modKeys.map((m) => (
                        <span key={m} className="text-[10px] px-1 py-0.5 bg-purple-50 text-purple-700 rounded truncate max-w-[120px]">{m}</span>
                      ))}
                  ...
                  <td className="px-3 py-2.5 text-gray-500 text-xs hidden 2xl:table-cell">
