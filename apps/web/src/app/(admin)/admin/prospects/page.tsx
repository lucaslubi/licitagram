    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <a
          ...
          className="inline-flex items-center gap-2 px-4 py-2 bg-brand text-white rounded-lg text-sm font-medium hover:bg-brand-dark transition-colors shrink-0"
        >
      ...
      <form className="mb-6 flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        {(params.search || params.uf || params.porte || params.min_participacoes) && (
          <a href="/admin/prospects" className="px-4 py-2 border rounded-md text-sm text-muted-foreground hover:bg-secondary">
            Limpar
          </a>
        )}
      </form>
      ...
      {error && (
        <div className="p-4 bg-red-900/20 border border-red-800 rounded-lg text-sm text-destructive mb-4">
          Erro: {error.message}
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead className="bg-card border-b">
            <tr>
              ...
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden sm:table-cell">CNPJ</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden md:table-cell">Email</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden md:table-cell">Telefone</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden lg:table-cell">Porte</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden lg:table-cell">UF</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden xl:table-cell">CNAE</th>
              ...
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden xl:table-cell">UFs Atuacao</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden xl:table-cell">Modalidades</th>
              <th className="text-left px-3 py-3 font-medium text-muted-foreground hidden 2xl:table-cell">
          ...
          <tbody className="divide-y divide-[#2d2f33]">
            {(competitors || []).map((c) => {
              ...
              return (
                <tr key={c.cnpj} className="hover:bg-secondary">
                  <td className="px-3 py-2.5 font-medium text-white max-w-[200px] truncate" title={c.razao_social || ''}>
                    {c.razao_social || '—'}
                  </td>
                  <td className="px-3 py-2.5 text-muted-foreground font-mono text-xs hidden sm:table-cell">{c.cnpj}</td>
                  <td className="px-3 py-2.5 hidden md:table-cell max-w-[180px] truncate" title={c.email || ''}>
                    {c.email ? (
                      <a href={`mailto:${c.email}`} className="text-foreground hover:underline text-xs">{c.email}</a>
                    ) : (
                  ...
                  <td className="px-3 py-2.5 text-muted-foreground text-xs hidden md:table-cell">
                    {c.telefone || <span className="text-muted-foreground/80">—</span>}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-muted-foreground hidden lg:table-cell">{c.uf || '—'}</td>
                  <td className="px-3 py-2.5 text-muted-foreground hidden xl:table-cell">{c.cnae_grupo || c.cnae_divisao || '—'}</td>
                  <td className="px-3 py-2.5 text-center font-semibold">{c.total_participacoes || 0}</td>
                  <td className="px-3 py-2.5 text-center text-brand font-semibold">{c.total_vitorias || 0}</td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`font-semibold ${
                      Number(c.win_rate) >= 0.5 ? 'text-brand' : Number(c.win_rate) >= 0.2 ? 'text-amber-400' : 'text-muted-foreground'
                    }`}>
                      {formatPercent(c.win_rate)}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-right text-muted-foreground/80 hidden md:table-cell">
                  ...
                  <td className="px-3 py-2.5 hidden xl:table-cell">
                    <div className="flex flex-wrap gap-0.5">
                      {ufsKeys.map((uf) => (
                        <span key={uf} className="text-[10px] px-1 py-0.5 border border-border bg-secondary text-foreground rounded">{uf}</span>
                      ))}
                      {Object.keys(ufsAtuacao).length > 5 && (
                        <span className="text-[10px] text-muted-foreground">+{Object.keys(ufsAtuacao).length - 5}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-3 py-2.5 hidden xl:table-cell">
                    <div className="flex flex-wrap gap-0.5 max-w-[150px]">
                      {modKeys.map((m) => (
                        <span key={m} className="text-[10px] px-1 py-0.5 border border-border bg-muted text-foreground rounded truncate max-w-[120px]">{m}</span>
                      ))}
                      {Object.keys(modalidades).length > 3 && (
                        <span className="text-[10px] text-muted-foreground">+{Object.keys(modalidades).length - 3}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-3 py-2.5 text-muted-foreground text-xs hidden 2xl:table-cell">
